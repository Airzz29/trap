...
app.use(express.static(__dirname));   // serve from root, not /public
...
function isBlocked(name) {
  ...
  const passL = n.replace(/[@4]/g,'a').replace(/[1!|]/g,'l').replace(/\+/g,'t').replace(/#/g,'h');
  const passI = n.replace(/[@4]/g,'a').replace(/[1!]/g,'i').replace(/\+/g,'t').replace(/#/g,'h');
...
app.post('/api/submit', (req, res) => {
  ...
  if (!name || !from) return res.status(400).json({ error: 'Name and location are required.' });
...
app.post('/api/admin/questions', adminOnly, (req, res) => {
  ...
  const q = { id: Date.now().toString(), text: text.trim(), type: type || 'text', req: required || 'yes', opts: opts || [] };
...
app.listen(PORT, () => console.log(`Trap House running on port ${PORT}`));
